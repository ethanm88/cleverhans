'''
delete/ reconcile with other file
'''
import pickle
import pprint
...
class Attack:
    def __init__(self, sess, batch_size=1,
                 lr_stage1=100, lr_stage2=1, num_iter_stage1=2000, num_iter_stage1_robust=4000, num_iter_stage2=6000,
                 th=None,
                 psd_max_ori=None):
    ...
    with tf.device("/gpu:0"):  # changed
        ...
        with tf.Session(config=tfconf) as sess:
            ...
            for l in range(num_loops):
                ...
                rescales = (np.max(np.abs(adv), axis=1))/(FLAGS.initial_bound) # + FLAGS.max_delta
