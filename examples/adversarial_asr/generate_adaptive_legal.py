        ...
        for i in range(MAX):
            ...
            for ii in range(self.batch_size):
                ...
                if i == 1:
                    ...
                    graph_data_2 = {'apply_delta': np.transpose(np.abs(librosa.core.stft(apply_delta[ii, :], center=False))), 'psd_thresh': thresholdPSD(self.batch_size, sess.run((self.th), feed_dict), sess.run((self.ori_input_tf), feed_dict), 2048.), 'unclipped': self.unclip(apply_delta[ii, :], sess.run((self.ori_input_tf)[ii], feed_dict), 2048.)}
